...
Seeing as zeroes of the Bernoulli numbers would thus correspond with zeroes of the Riemann zeta function, I spent much of my junior year of high school trying to find an expression for $B_n$ for non-integer values of $n$, for the purpose of studying the Riemann hypothesis.
...
$$B_n=lim_{a \to 0} \left. {\frac{d^n}{dz^n} \frac{z}{e^z-1}} \right|_{z=a} \\quad [\mathrm{II}]$$
...
$$\frac{z}{e^z-1} = \sum\limits_{j=0}^\infty {f^{(j)}(k) \frac{(z-k)^j}{j!}} \\quad [\mathrm{I}]$$
...
$$\frac{d^n x^j}{dx^n} = \frac{j ! x^{j-n}}{(j-n)!} \\quad [\mathrm{II}]$$
...
$$\frac{d^n x^j}{dx^n} = \frac{\Gamma (j + 1) x^{j-n}}{\Gamma (j-n+1)} \\quad [\mathrm{III}]$$
...
$$B_n=lim_{z \to 0} \left. \sum\limits_{j=0}^\infty {f^{(j)}(k) \frac{\Gamma (j+1)(z-k)^{j-n}}{j! \Gamma(j-n+1)}} \right.   \\quad [\mathrm{IV}]$$
...
$$\frac{d^n e^{\alpha x}}{dx^n} = \alpha^n e^{\alpha x} \\quad [\mathrm{I}]$$
...
$$f(z)=\frac{1}{2T}\sum \limits_{j=-\infty}^\infty {e^{ijz} \int_{-T}^{T} {f(t)e^{-ijt}dt}}, z \in (-T, T) \\quad [\mathrm{II}]$$
...
$$\frac{z}{e^z-1}=\frac{1}{2T}\sum \limits_{j=-\infty}^\infty {e^{ijz} \int_{-T}^{T} {\frac{te^{-ijt}}{e^t-1}dt}}, z \in (-T, T) \\quad [\mathrm{III}]$$
...
$$B_n=lim_{z \to 0} \left. {\frac{1}{2T}\sum \limits_{j=-\infty}^\infty {(ij)^n e^{ijz} \int_{-T}^{T} {\frac{te^{-ijt}}{e^t-1}dt}}} \right. \\quad  [\mathrm{IV}]$$
